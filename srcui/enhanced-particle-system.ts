import { Container, Graphics, Filter, BlurFilter, ColorMatrixFilter } from 'pixi.js';
import { GAME_CONFIG } from '../src/utils/constants';
import { Position, MathUtils, ColorUtils, PerformanceUtils } from '../src/utils/helpers';
...
export class EnhancedParticleSystem
{
  ...
  private particlePool: ReturnType<typeof PerformanceUtils.createObjectPool>;
  ...
  constructor ()
  {
    ...
    this.blurFilter = new BlurFilter( { strength: 1, quality: 2 } );
  ...
  // Environment effects
  public createSnowEffect ( bounds: { width: number; height: number; } ): void
  {
    if ( !this.isEnabled ) return;
    ...
    for ( let i = 0; i < 3; i++ )
    {
      ...
      particle.y = -10;
      particle.vx = MathUtils.randomRange( -0.5, 0.5 );
      particle.vy = MathUtils.randomRange( 0.5, 1.5 );
      particle.life = 600; // Long-lived
      particle.maxLife = particle.life;
      particle.size = MathUtils.randomRange( 1, 3 );
      particle.color = 0xFFFFFF;
      particle.gravity = 0.01;
      particle.type = ParticleType.SNOW;
  ...
  public getPoolInfo (): { active: number; poolSize: number; }
  {
    return {
